...
long long arrayManipulation(int n, int queries_rows, int queries_columns, int** queries){
    ...
    for(int i=1;i<=queries_rows;i++){
        arr[queries[i-1][0]]+=queries[i-1][2];
        arr[queries[i-1][1]+1]-=queries[i-1][2];
    }
    long long max=0;
    long long sum=0;
    for(int i=1;i<=n;i++){
